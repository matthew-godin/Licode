...
public class Win {
    public static void win(UserRepository userRepository, Map<String, String> sids, Map<String, List<QuestionInformation>> sidsQuestions, Map<String, Integer> sidsProgress, Map<String, String> matches, String sid) {
        ...
        user.setEloRating(user.getEloRating() + Math.floor((user.getNumWins() + user.getNumLosses() < 30 ? (user.getEloRating() < 2300 ? 40 : 20) : (user.getHas2400RatingHistory() ? 10 : 20)) * eloRatingVariation));
        opponent.setNumLosses(opponent.getNumLosses() + 1);
        opponent.setEloRating(opponent.getEloRating() - Math.ceil((opponent.getNumWins() + opponent.getNumLosses() < 30 ? (opponent.getEloRating() < 2300 ? 40 : 20) : (opponent.getHas2400RatingHistory() ? 10 : 20)) * eloRatingVariation));
